...
func NewGraphicContext(img draw.Image) *ImageGraphicContext {
	...
	case *image.Paletted:
		painter = raster.NewPalettedPainter(selectImage)
	default:
...
func (gc *ImageGraphicContext) ClearRect(x1, y1, x2, y2 int) {
	switch dst0 := gc.img.(type) {
	case *image.Paletted:
		colorIndex := gc.painter.(*raster.PalettedPainter).GetColorIndex(gc.Current.FillColor)
		b := dst0.Bounds()
		if x1 < b.Min.X {
			x1 = b.Min.X
		}
	...
	default:
		// jdc - not sure why Over was chosen instead of Src???

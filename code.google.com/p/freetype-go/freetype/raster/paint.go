...
type PalettedPainter struct {
	// The image to compose onto.
	Image *image.Paletted
	// The index of the color to paint the spans.
	idx uint8
	// cached color of last color that was looked up
	prevColor color.Color
}

func NewPalettedPainter(m *image.Paletted) *PalettedPainter {
	return &PalettedPainter{Image: m, idx: 0, prevColor: nil}
}

func (r *PalettedPainter) Paint(ss []Span, done bool) {
	b := r.Image.Bounds()
	for _, s := range ss {
		if s.Y < b.Min.Y {
			continue
		...
		base := (s.Y-r.Image.Rect.Min.Y)*r.Image.Stride - r.Image.Rect.Min.X
		pix := r.Image.Pix[base+s.X0 : base+s.X1]
		for i := range pix {
			pix[i] = r.idx //color
		}
...
func (r *PalettedPainter) SetColor(c color.Color) {
	if c != r.prevColor {
		r.idx = uint8(r.Image.Palette.Index(c))
		r.prevColor = c
	}
}

func (r *PalettedPainter) GetColorIndex(c color.Color) uint8 {
	r.SetColor(c)
	return r.idx
}
